...
def home(request):
    if request.user.is_authenticated:
        return redirect(f'/dashboard/{request.user._id}/')
...
def manage_order(request, object_id):
    ...
    context = {
        'orders_request':zip(Basket.objects.exclude(status = 'Created'),all_order_items),
    ...
    if request.method == 'POST':
        ...
        for k,v in d.items():
            if k == 'accept':
                ...
                "type": "notify_manager",
                "message_accept": f"food request:{message}",
                })
            if k == 'ready':
                basket = Basket.objects.get(id = v )
                basket.status = 'Ready'
            ...
            if k == 'decline':
                ...
                async_to_sync(channel_layer.group_send)("notification", {
                "type": "notify_manager",
                "message_decline": f"food request:{message}",
                })
            if k == 'cash':
                basket = Basket.objects.get(id = v )
